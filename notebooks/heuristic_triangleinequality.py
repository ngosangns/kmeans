		...
		for centroid in self.centroidList:
			if pos != currCentroid:
				if 0.5*self.interClusterDistance[closestCentroid][pos] < minDist:
					...
					if len(self.centroidDistance[point.id]) < self.centroidsToRemember:
						bisect.insort(self.centroidDistance[point.id], (self.interClusterDistance[closestCentroid][pos] - minDist,pos))
					elif self.centroidDistance[point.id][self.centroidsToRemember-1][0] > (self.interClusterDistance[closestCentroid][pos] - minDist):
						bisect.insort(self.centroidDistance[point.id], (self.interClusterDistance[closestCentroid][pos] - minDist,pos))
						del self.centroidDistance[point.id][self.centroidsToRemember]
			pos += 1
			...
			if x != point.centroid:
				if self.upperBound[point.id] > self.lowerBound[point.id][x]:
					if self.upperBound[point.id] > 0.5*self.interClusterDistance[closestCentroid][x]:
						if minDist > self.lowerBound[point.id][x] or minDist > 0.5*self.interClusterDistance[closestCentroid][x]:
							dist = self.getDistance(point,centroid.point)
...
# config1= Kmeans(numClusters,pointList,1000,40)
...
# start = time.time()
# #self, k, pointList, kmeansThreshold, predictionThreshold, isPrediction = 0, initialCentroids = None
# config2= heuristic1e.Kmeans(numClusters,pointList,1000,40,cList)
# print ("Time taken:",time.time() - start)
...
# start = time.time()
# #self, k, pointList, kmeansThreshold, predictionThreshold, isPrediction = 0, initialCentroids = None
# config2= triangleInequality.Kmeans(numClusters,pointList,1000,cList)
# print ("Time taken:",time.time() - start)

# start = time.time()
# #self, k, pointList, kmeansThreshold, predictionThreshold, isPrediction = 0, initialCentroids = None
# config2= egyptian.Kmeans(numClusters,pointList,1000,cList)
# print ("Time taken:",time.time() - start)
